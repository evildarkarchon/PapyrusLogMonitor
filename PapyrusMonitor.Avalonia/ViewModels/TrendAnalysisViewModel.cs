...
public class TrendAnalysisViewModel : ViewModelBase
{
    ...
    private async Task RefreshAnalysisAsync()
    {
        ...
        try
        {
            ...
            StatusMessage = $"Analysis complete - {statistics.Count} data points over {CurrentAnalysis.TimeRange.Duration:hh\\:mm\\:ss}";
            HasData = true;
        }
        finally
        {
    ...
    private PlotModel CreatePlotModel(string title, TrendData trendData, OxyColor color)
    {
        ...
        if (trendData.Summary != null)
        {
            ...
            {
                Text = $"Min: {trendData.Summary.Min:F1}, Max: {trendData.Summary.Max:F1}, Avg: {trendData.Summary.Average:F1}",
